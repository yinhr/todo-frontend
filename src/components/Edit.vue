    ...
    <div v-if="confirming" class="fixed inset-0 flex justify-center px-4 min-h-screen z-50">
      <div class="self-center bg-white rounded text-black px-8 py-4">
        ...
        <div class="py-2 flex justify-around">
          <button class="text-white py-1 px-2 bg-blue-400 hover:bg-blue-500 rounded" @click="putTodo">更新</button>
    ...
    <div class="min-h-screen py-8 w-full max-w-2xl mx-auto">
      <div class="px-4">
        ...
        <div class="flex py-2">
          ...
          <h1 class="text-xl self-center">タスクを編集</h1>
        </div>
        <div class="py-2">
          <label for="taskTitle"></label>
          <input type="text" id="taskTitle" v-model="todo.title" class="w-full p-1 block rounded border-2 border-gray-400 focus:border-green-400 focus:outline-none" placeholder="タスク名">
        </div>
        ...
        <button class="text-white py-1 px-2 bg-blue-400 hover:bg-blue-500 rounded" @click="confirmSubmit">更新</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getSuccessful: function(response) {
      if (response.status === 200) {
        this.todo = response.data
        this.tasks = this.todo.tasks.map(v => {
          let rtasks = {};
          rtasks['id'] = this.taskid++;
    ...
    putTodo: function() {
      if (this.validForm()) {
        ...
        TodoRepository.put(this.todo)
          .then(response => this.putSuccessful(response))
          .catch(error => this.failed(error))
      }
    ...
    putSuccessful: function(response) {
      if (response.status == 200) {
        this.pushMessage("タスクを更新しました", this.$store.state.info)
      }
    },
